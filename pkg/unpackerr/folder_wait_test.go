...
func TestWaitExtensionsIgnoresNestedFiles(t *testing.T) {
	...
	got := unpack.Map[item]
	if got == nil {
		got = unpack.Map[filepath.Join(item, "movie.rar")]
	}
	if got == nil || got.Note != "" {
...
func TestQueueFromExtractWatchMode(t *testing.T) {
	...
	archive := filepath.Join(item, "movie.rar")
	...
	itemKey := item
	if unpack.Map[itemKey] == nil {
		itemKey = archive
	}

	got := unpack.queueFromExtract(itemKey, unpack.Map[itemKey])
	...
	pollArchive := filepath.Join(pollItem, "movie.rar")
	if err := os.WriteFile(pollArchive, []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	pollKey := pollItem
	if pollUnpack.Map[pollKey] == nil {
		pollKey = pollArchive
	}

	got = pollUnpack.queueFromExtract(pollKey, pollUnpack.Map[pollKey])
