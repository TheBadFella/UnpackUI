...
func (u *Unpackerr) logWebserver(printf configLine) {
	...
	printf(" => Starting webserver. Listen address: http%s://%v%s%s (%d upstreams) auth:%s",
		ssl, u.Webserver.bindAddr(), u.Webserver.URLBase, featStr, len(u.Webserver.Upstreams), u.uiPassword().Type())
