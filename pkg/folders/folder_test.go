...
func TestPollerStartupScanKeepsRecursiveArchiveTracking(t *testing.T) {
	t.Parallel()
	...
	archive := filepath.Join(nested, "movie.zip")
	if err := os.WriteFile(archive, []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if len(paths) != 1 || paths[0] != archive {
		t.Fatalf("startup scan paths=%v, want %s", paths, archive)
	}
	if tracker.Folders[archive] == nil {
		t.Fatalf("poller startup scan did not track archive: %s", archive)
	}
	if _, ok := tracker.WatchDirs[nested]; !ok {
		t.Fatalf("poller startup scan did not register nested watch: %s", nested)
	}
}

func TestStartupScanPrefersNestedWatchConfig(t *testing.T) {
	t.Parallel()
	...
	tracker, err := (WatchConfig{Buffer: 8}).NewWatcher(
		[]*FolderConfig{parentCfg, childCfg}, noopLogger{}, 1, "",
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := tracker.Folders[archive].Config; got != childCfg {
		t.Fatalf("startup scan config = %q, want child %q", got.Path, child)
	}
}
