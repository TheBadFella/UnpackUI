...
| `UN_STATE_FILE` | automatic | Stores watched-folder recovery state so interrupted work can resume after restart. Use a writable persistent path, or `off` to disable recovery. |
...
| `UN_WEBSERVER_UI` | `false` | Enables the status dashboard, its detailed status API, WebSocket updates, and clear-completed action. |
...
These are additions relative to the upstream commit merged for UnpackUI 1.6.1.
Settings such as `UN_REMNANT_ACTION` are inherited from upstream, even if they
first appear in this release.
...
| `UN_REMNANT_ACTION` | Controls upstream interrupted-extraction remnant handling (`rename`, `remove`, or `off`). |
